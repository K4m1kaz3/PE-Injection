...
void LoadExe(BYTE* data){
	...
	printf("[*] Base relocation by reloc table\n");
	//fixing .reloc table
	...
	for (int section = 0; section < nt_header->FileHeader.NumberOfSections; section++) {
		...
		if (sec_header->Characteristics & 0xC0000000) //read- write
...
int main(int argc, char** argv) {
	if (argc != 2) {
		printf("Usage : Pe Injection.exe [exe file]\n");
		return 1;
	}
	HANDLE hFile = CreateFileA(argv[1], GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING,
